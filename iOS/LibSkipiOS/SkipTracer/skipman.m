...
- (void) openSkipFile
{
    //char *skipFilePath = (char *)[[NSHomeDirectory() stringByAppendingPathComponent:@SKIP_FILE_NAME] UTF8String];
    char *skipFilePath = (char *)[[[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:@SKIP_FILE_NAME] UTF8String];
    
    skipFile = fopen(skipFilePath, "r+");
    if (!skipFile) {
        skipFile = fopen(skipFilePath, "w+");
        if (!skipFile) {
